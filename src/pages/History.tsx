  ...
  return (
    <SidebarProvider>
      <div className="min-h-screen flex w-full bg-gradient-to-b from-background to-background/95">
        <AppSidebar />
        <div className="flex-1 p-2 space-y-3 md:p-8 md:space-y-8">
          <header className="space-y-1">
            <h1 className="text-xl md:text-3xl font-bold tracking-tight gradient-text flex items-center gap-2">
              <HistoryIcon className="h-5 w-5 md:h-8 md:w-8 text-primary" />
              Trading History
            </h1>
            <p className="text-xs md:text-base text-muted-foreground">View your closed positions and trading performance</p>
          </header>
          ...
          <Card className="glass-effect overflow-hidden">
            ...
            <CardContent className="p-0">
              <div className="space-y-2 max-h-[calc(100vh-16rem)] overflow-y-auto custom-scrollbar px-2 pb-3 md:px-6 md:pb-6">
                {isLoading ? (
                  ...
                  <div className="text-center text-muted-foreground py-6 glass-effect rounded-lg p-3 md:p-6">
                    <Calendar className="h-8 w-8 md:h-12 md:w-12 mx-auto mb-2" />
                  ...
                  closedPositions.map((position) => {
                    ...
                    return (
                      <div
                        key={position.id}
                        className="glass-effect p-2.5 md:p-4 rounded-lg hover:bg-card/40 transition-all duration-300 space-y-2"
                      >
                        <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-1.5">
                          <div className="space-y-1">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="font-semibold text-sm md:text-lg">{position.symbol}</span>
                              <Badge 
                          ...
                          <div className={`flex items-center gap-1.5 ${
                            ...
                            {isProfitable ? (
                              ...
                              <TrendingDown className="h-3.5 w-3.5 md:h-5 md:w-5" />
                            )}
                        ...
                        <div className="grid grid-cols-3 gap-1.5 pt-1.5 border-t border-white/10 text-xs md:text-sm">
                          <div>
